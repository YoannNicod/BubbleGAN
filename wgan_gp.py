...
# Trick to make the code work with Tensorflow 2
###################################################
...
class WGANGP():
    ...
    def build_generator(self):
        ...
        model.add(BatchNormalization(momentum=0.8))
        model.add(Activation("relu"))        
        model.add(Conv2D(self.channels, kernel_size=3, padding="same", use_bias=False))
    ...
    def build_critic(self):
        ...
        model.add(Conv2D(32, kernel_size=3, strides=2, input_shape=self.img_shape, padding="same", use_bias=False))
    ...
    def train(self, epochs, batch_size, sample_interval=50):
        ...
        X_train = loadFiles(data_directory)
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("Images_wgan_gp/bubble_%d.png" % epoch)
        plt.close()
...
if __name__ == '__main__':
    ...
    wgan = WGANGP()#30000, 32, 100
    wgan.train(epochs=50000, batch_size=32, sample_interval=200)